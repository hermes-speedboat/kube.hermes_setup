...
When `configuration_answers` already exists, starting `./configure.sh` interactively offers to reuse it. Answer `n` to start a fresh wizard, or use `--from-answers` to select replay explicitly without the question.
